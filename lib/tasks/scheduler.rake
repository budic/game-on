  ...
  task process_weekly: :environment do
    ...
    #RecurringEvent.all.each do |r| 
      ...
      stime = Time.zone( time.year, time.month, time.day, r.start_time.hour, r.start_time.min ) + doffset.days
      etime = Time.zone( time.year, time.month, time.day, r.end_time.hour, r.end_time.min ) + doffset.days
